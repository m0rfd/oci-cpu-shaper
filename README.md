...
- `pkg/adapt/` – Adaptive controller and mode handling kept in `controller_loop.go`, `controller_step.go`, `controller_state.go`, `state.go`, `suppression.go`, `config.go`, and the dry-run adapter in `noop_controller.go`.
