...
3. Use the provided tooling shortcuts before submitting changes and keep the ≥98% statement coverage guarantee in place:
   ...
   - `make test` to execute the unit suite with the Go race detector enabled. Set `RUN_E2E_TESTS=1 make test` when you need the tagged CLI end-to-end harness that exercises the mock IMDS/Monitoring servers in `tests/e2e`. The environment flag stays unset in untrusted contexts (for example, pull requests from forks) so the suite only runs when explicitly requested.
   - `make test-all` to run the unit, integration, and tagged E2E suites together, write `coverage-integration.out`/`coverage-e2e.out`, and merge every profile into `coverage.out`/`coverage.txt` with `gocovmerge`. CI’s `check` job now uses this target so the ≥98% coverage guardrail reflects all harnesses.
   - `make coverage MIN_COVERAGE=98` to confirm the repository-wide coverage threshold documented in §11 of the implementation plan. The target merges unit, integration, and optional tagged E2E profiles; use `RUN_E2E_TESTS=1 KEEP_E2E_COVERAGE=1 make coverage` when you need the E2E profile preserved as `coverage-e2e.out` for reuse in CI.
   - `make check` to run linting, formatting, the full `test-all` flow, coverage enforcement, CodeQL, and agent verification in one pass. Set `CHECK_INCLUDE_CODEQL=0 make check` if you want to mirror CI’s faster path where CodeQL runs in its dedicated workflow.
