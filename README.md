...
3. Use the provided tooling shortcuts before submitting changes and keep the ≥98% statement coverage guarantee in place:
   ...
   - `make codeql-setup` to install the CodeQL CLI into a versioned toolcache directory, retain existing installs, and prefetch the default query packs into `.cache/codeql/packs` for offline analysis. `make codeql`, `make codeql-actions`, `make codeql-go`, or `make codeql-all` mirror the PR CodeQL checks directly, writing databases to `.cache/codeql/databases`, emitting SARIF artifacts to `artifacts/codeql/`, and honoring `CODEQL_DATABASE_ROOT`, `CODEQL_CACHE_DIR`, and `CODEQL_PACK_CACHE_DIR` overrides. Prime `.cache/gomod` (and `.cache/go`) with `make go-mod-download`—or run `GOCACHE=.cache/go GOMODCACHE=.cache/gomod go mod download -mod=readonly` followed by `go mod verify -mod=readonly`—before running the CodeQL targets so the analysis uses vendored or pre-downloaded modules. Go analysis uses the baked-in build command with `-mod=readonly -trimpath`, runs `codeql database analyze` with `--threads=0`, and you can override the query suites with `CODEQL_ACTIONS_QUERY_PACK`/`CODEQL_GO_QUERY_PACK` when experimenting locally. Use `make codeql-clean` to prune generated databases and SARIF outputs without disturbing the cached CLI or packs.
