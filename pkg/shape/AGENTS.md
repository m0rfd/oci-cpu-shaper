...
- Pause/resume thresholds stay in `pause.go`, and duty-cycle helpers that manage tight loops remain in `busywait.go`; mirror every helper with dedicated tests to maintain the ≥98% coverage target from the root AGENT.
- Share fixtures through `testhelpers_test.go` and keep build-tagged behavior isolated so rootless builds remain unaffected.
- Follow the repo QA workflow referenced in the root AGENT by rerunning `make lint` (or `make lint-fix`), `make test`, and `make coverage MIN_COVERAGE=98` after changes and documenting any new tunables.
