...
- Metrics client builders, context helpers, and the instance-principal wrapper now live in `metricsclient/`; keep that package focused so CLI layers only orchestrate invocations.
...
- Per the repo-wide QA rules (root AGENT), rerun `make lint` (or `make lint-fix`), `make test`, and `make coverage MIN_COVERAGE=97` before sending patches and refresh docs when API contracts shift.
