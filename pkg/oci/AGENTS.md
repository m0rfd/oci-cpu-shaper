...
- Per the repo-wide QA rules (root AGENT), rerun `make lint` (or `make lint-fix`), `make test`, and `make coverage MIN_COVERAGE=98` before sending patches and refresh docs when API contracts shift.
