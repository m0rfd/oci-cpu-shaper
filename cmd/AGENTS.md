...
- Metrics client selection now lives in `pkg/oci/metricsclient`; keep CLI glue limited to picking builders/offline paths instead of rebuilding factories here.
...
- Hold coverage ≥98% (`make coverage`) and add tests for every new flag path before merging.
