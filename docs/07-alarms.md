...
4. Hit the Prometheus endpoint directly (`curl -fsS ${HTTP_ADDR:-http://127.0.0.1:9108}/metrics`) and confirm the `shaper_mode` (controller state), `shaper_enforcement_mode`, and `oci_p95` samples line up with the alarm evaluation window; §9.5 includes a canonical scrape example.
