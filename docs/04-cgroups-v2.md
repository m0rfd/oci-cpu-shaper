...
- Pair these checks with the shaper’s `/metrics` output and MQL queries described in `docs/05-monitoring-mql.md`. The exporter publishes `shaper_target_ratio`, `duty_cycle_ms`, and `worker_count` so operators can spot drift between requested duty cycles and the active worker pool, alongside `shaper_mode` (controller state), `shaper_enforcement_mode`, `oci_p95`, `oci_last_success_epoch`, and `host_cpu_percent` for reconciling controller decisions with OCI telemetry and host contention. The same endpoint now exposes the detected cgroup files via `cgroup_cpu_weight`, `cgroup_cpu_max_quota`, `cgroup_cpu_max_period`, and `cgroup_cpu_max_unlimited` so dashboards can confirm the runtime weight/ceiling without shelling into the host.
- The `/healthz` endpoint on the same listener now reports both the controller `state` and `mode` plus a `cgroup` block that includes the parsed `cpu.weight`/`cpu.max` values or any error strings. Orchestrators can alarm on missing files or drifted weights without scraping `/sys/fs/cgroup` directly.
- Startup logs now include the detected cgroup path and emit a warning when the observed `cpu.weight` exceeds the low-weight defaults described above, providing an immediate signal when Compose/Quadlet manifests drift from the §6 guidance.
